...
# 🎯 RANGE TRAILING - Многоуровневая защита (v1.4.2)
# Формат: (макс_профит_для_порога, порог_отката)
RANGE_TRAILING_THRESHOLDS = [
    (0.003, 0.0010),  # До +0.3% прибыли: откат 0.10% (свободно, даём расти)
    (0.005, 0.0007),  # От +0.3% до +0.5%: откат 0.07% (средняя фиксация)
    (999, 0.0005)     # Выше +0.5%: откат 0.05% (жёсткая фиксация прибыли)
]

# Минимальное обновление TP (чтобы не спамить биржу)
RANGE_TRAILING_TP_UPDATE_THRESHOLD = 0.001  # 0.1% - обновлять TP только при значительном изменении пика
