...
class BotTrailingMixin:
    ...
    def check_range_trailing(self):
        ...
        🆕 v1.4.9: КРИТИЧЕСКИЕ ФИКСЫ:
        - АДАПТИВНАЯ активация: % от динамического TP (RANGE_TRAILING_ACTIVATION_RATIO)
    ...
    def _finalize_trailing_close(self, reason, exec_price, amount):
        ...
        # 🆕 КУЛДАУН: только после убыточной сделки!
        from datetime import datetime, timedelta
        if net_pnl > 0:
            # Прибыльное закрытие трейлингом - НЕТ КУЛДАУНА
            self.last_trade_time = datetime.now() - timedelta(hours=2)
        else:
            # Убыточное закрытие трейлингом - ВКЛЮЧАЕМ КУЛДАУН
